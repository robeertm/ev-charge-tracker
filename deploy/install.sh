...
# value and skips auto-discovery.
is_port_free() {
    local p="$1"
    if command -v ss &>/dev/null; then
        ! ss -tlnH "sport = :$p" | grep -q .
    else
...
if [ -z "$APP_PORT" ]; then
    if is_port_free 7654; then
        APP_PORT=7654
    else
        log "Port 7654 belegt — suche freien Port …"
        for p in $(seq 7655 7700); do
            if is_port_free "$p"; then
                APP_PORT="$p"
                break
            fi
        done
        [ -z "$APP_PORT" ] && die "Kein freier Port zwischen 7654 und 7700. Setze EV_APP_PORT=<n> manuell."
        ok "Gewählter Port: $APP_PORT (7654 war belegt)."
    fi
else
    ok "Vorgegebener Port (EV_APP_PORT): $APP_PORT"
fi
...
EOF
ok "systemd-Unit installiert: $UNIT_PATH (port $APP_PORT)"
...
install_tailscale() {
    ...
    if tailscale status --json 2>/dev/null | grep -q '"BackendState": "Running"'; then
        log "Aktiviere 'tailscale serve --https=443 → 127.0.0.1:$APP_PORT' …"
        tailscale serve reset 2>/dev/null || true
        tailscale serve --bg --https=443 "http://127.0.0.1:$APP_PORT" || warn "Tailscale Serve fehlgeschlagen."
